...
import com.inmobi.qa.falcon.generated.process.ExecutionType;
...
public class NewPrismProcessUpdateTest {
	...
	@Test(dataProvider = "DP", groups = {"multiCluster"}, dataProviderClass = Bundle.class,timeOut=1200000)
	...
	public void updateProcessRollStartTimeForwardInEachColoWithOneProcessRunning() throws Exception {
	...
	@Test(dataProvider = "DP", groups = {"multiCluster"}, dataProviderClass = Bundle.class,timeOut=1800000)
	public void updateProcessConcurrencyWorkflowExecutionInEachColoWithOneColoDown(Bundle bundle) throws Exception {
		...
		try {
			...
			String prismString = getResponse(prismHelper, UA2Bundle,true);
			Assert.assertEquals(Util.getProcessObject(prismString).getParallel(), initialConcurrency);
			Assert.assertEquals(Util.getProcessObject(prismString).getWorkflow().getPath(), "/examples/apps/phailFs/workflow.xml");
			...
			Assert.assertEquals(Util.getProcessObject(coloString).getWorkflow().getPath(), "/examples/apps/aggregator1");
	...
	public void updateProcessFrequencyInEachColoWithOneProcessRunning() throws Exception {
		...
		try {
			...
			String updatedProcess = instanceUtil.setProcessFrequency(UA2Bundle.getProcessData(), new Frequency(7, TimeUnit.minutes));
	...
	public void updateProcessConcurrencyInEachColoWithOneProcessRunning(Bundle bundle) throws Exception {
		...
		try {
			...
			while (!status.equalsIgnoreCase("SUCCEEDED") || !status.equalsIgnoreCase("FAILED") || !status.equalsIgnoreCase("DONEWITHERROR")) {
				int statusCount = instanceUtil.getInstanceCountWithStatus(UA3ColoHelper, Util.readEntityName(UA2Bundle.getProcessData()), org.apache.oozie.client.CoordinatorAction.Status.RUNNING, "PROCESS");
				...
				if(status.contains("No Jobs match your criteria!"))
					Assert.assertTrue(false,"message No Jobs match your criteria! should not been there");
				Thread.sleep(30000);
			}
	...
	public void updateProcessIncreaseValidityInEachColoWithOneProcessRunning() throws Exception {
		...
		try {
			...
			String newEndTime = instanceUtil.addMinsToTime(instanceUtil.dateToOozieDate(UA2Bundle.getProcessObject().getClusters().getCluster().get(0).getValidity().getEnd()), 4);
	...
	public void updateProcessConcurrencyInEachColoWithOneColoDown(Bundle bundle) throws Exception {
		...
		try {
			...
			String prismString = getResponse(prismHelper, UA2Bundle,true);
			dualComparisonFailure(UA2Bundle, UA3ColoHelper);
			Assert.assertEquals(Util.getProcessObject(prismString).getParallel(), UA2Bundle.getProcessObject().getParallel());
	...
	@Test( groups = {"multiCluster"},timeOut=1200000)
	public void updateProcessConcurrencyExecutionWorkflowInEachColoWithOneProcessRunning() throws Exception {
		Bundle b = new Bundle();
		b = (Bundle)Bundle.readBundle("src/test/resources/updateBundle")[0][0];
		b .generateUniqueBundle();
		Bundle UA1Bundle = new Bundle(b, UA1ColoHelper);
	...
	@Test(groups = {"multiCluster"},timeOut=1200000)
	public void updateProcessConcurrencyExecutionWorkflowInEachColoWithOneProcessSuspended() throws Exception {
		Bundle b = new Bundle();
		b = (Bundle)Bundle.readBundle("src/test/resources/updateBundle")[0][0];
		b .generateUniqueBundle();
		Bundle UA1Bundle = new Bundle(b, UA1ColoHelper);
	...
	@Test(groups = {"multiCluster"},timeOut=1200000)
	public void updateProcessFrequencyInEachColoWithOneProcessRunning_dailyToMonthly_withStartChange() throws Exception {
		Bundle b = (Bundle)Bundle.readBundle("src/test/resources/updateBundle")[0][0];
		b .generateUniqueBundle();
		Bundle UA1Bundle = new Bundle(b, UA1ColoHelper);
		Bundle UA2Bundle = new Bundle(b, UA2ColoHelper);
	...
	private void dualComparisonFailure(Bundle bundle, ColoHelper coloHelper) throws Exception {
		AssertJUnit.assertNotSame(getResponse(prismHelper, bundle,true), getResponse(coloHelper, bundle,true));

	}

	private String getResponse(PrismHelper prismHelper, Bundle bundle, boolean bool) throws Exception {
	...
	private void verifyNewBundleCreation(ColoHelper coloHelper, String originalBundleId, int originalBundleCount, String processName, boolean shouldBeCreated) throws Exception {
		...
		if (shouldBeCreated) {
			...
			AssertJUnit.assertEquals("eeks! new bundle is getting created!!!!",newBundleId, originalBundleId);
	...
	public ServiceResponse updateProcessFrequency(Bundle bundle, com.inmobi.qa.falcon.generated.dependencies.Frequency frequency) throws Exception {
		String oldData = new String(bundle.getProcessData());
		Process updatedProcess = bundle.getProcessObject();
	...
	private Frequency getRandomFrequency(Bundle bundle) throws Exception {
		com.inmobi.qa.falcon.generated.dependencies.Frequency current = bundle.getProcessObject().getFrequency();
