...
import com.inmobi.qa.falcon.generated.process.Retry;
...
public class NewRetryTest {
   ...
   @Test(dataProvider="DP",groups={"0.2.2","retry"},enabled=true)
    public void testRetryInProcessZeroAttemptUpdate(Bundle bundle,String retryType,int delay,String delayUnits,int retryAttempts) throws Exception
    {
...
//        List<String> initialData=Util.getHadoopLateData(UA3ColoHelper,Util.getInputFeedFromBundle(bundle));
    	...
    	{
    		Util.assertSucceeded(prismHelper.getFeedHelper().submitEntity(URLS.SUBMIT_URL,data));
    	}
    	...
    	//ServiceResponse response=prismHelper.getProcessHelper().submitAndSchedule(URLS.SUBMIT_AND_SCHEDULE_URL,bundle.getProcessData());
        ServiceResponse response=prismHelper.getProcessHelper().submitEntity(URLS.SUBMIT_URL,bundle.getProcessData());
        ...
        if(retryAttempts<=0 || delay<=0)
        {
            ...
            String bundleId=Util.getCoordID(Util.getOozieJobStatus(UA3ColoHelper,Util.readEntityName(bundle.getProcessData()),"NONE").get(0));
            ...
            com.inmobi.qa.falcon.generated.process.Process oldProcessObject=bundle.getProcessObject();
            ...
            prismHelper.getProcessHelper().update((bundle.getProcessData()),bundle.getProcessData());
            //Assert.assertFalse(prismHelper.getProcessHelper().update(Util.readEntityName(bundle.getProcessData()),bundle.getProcessFilePath()).contains("updated successfully"),"process was not updated successfully");
            String newBundleId=instanceUtil.getLatestBundleID(UA3ColoHelper,Util.readEntityName(bundle.getProcessData()),"process");
    ...
    @Test(dataProvider="DP",groups={"0.2.2","retry"},enabled=true)
    public void testRetryInProcessLowerAttemptUpdate(Bundle bundle,String retryType,int delay,String delayUnits,int retryAttempts) throws Exception
    {
        bundle=new Bundle(bundle,UA3ColoHelper);
        displayInputs("testRetryInProcessUpdate", delay, retryType, retryAttempts);
        try {
...
//        List<String> initialData=Util.getHadoopLateData(UA3ColoHelper,Util.getInputFeedFromBundle(bundle));
        ...
        if(retryAttempts<=0 || delay<=0)
        {
            ...
            while(!validateFailureRetries(UA3ColoHelper,getDefaultOozieCoord(UA3ColoHelper,bundleId),1))
            {
                //wait
            }
            ...
            if((retryAttempts-2)>0)
            {
                ...
                Assert.assertTrue(prismHelper.getProcessHelper().update((bundle.getProcessData()),bundle.getProcessData()).getMessage().contains("updated successfully"),"process was not updated successfully");
                String newBundleId=instanceUtil.getLatestBundleID(UA3ColoHelper,Util.readEntityName(bundle.getProcessData()),"process");
    ...
    public void testRetryInProcessLowerManageableAttemptUpdate(Bundle bundle,String retryType,int delay,String delayUnits,int retryAttempts) throws Exception
    {
        bundle=new Bundle(bundle,UA3ColoHelper);
        displayInputs("testRetryInProcessUpdate", delay, retryType, retryAttempts);
        try {
...
//        List<String> initialData=Util.getHadoopLateData(UA3ColoHelper,Util.getInputFeedFromBundle(bundle));
    ...
    @Test(dataProvider="DP",groups={"0.2.2","retry"},enabled=false)
    public void testRetryInProcessLowerBoundaryAttemptUpdate(Bundle bundle,String retryType,int delay,String delayUnits,int retryAttempts) throws Exception
    {
        bundle=new Bundle(bundle,UA3ColoHelper);
        displayInputs("testRetryInProcessUpdate", delay, retryType, retryAttempts);
        try {
...
//        List<String> initialData=Util.getHadoopLateData(UA3ColoHelper,Util.getInputFeedFromBundle(bundle));
        ...
        if(retryAttempts<=0 || delay<=0)
        {
            ...
            while(!validateFailureRetries(UA3ColoHelper,getDefaultOozieCoord(UA3ColoHelper,bundleId),2))
            {
                //wait
            }
    ...
    @Test(dataProvider="DP",groups={"0.2.2","retry"},enabled=false)
    public void testRetryInProcessUpdate(Bundle bundle,String retryType,int delay,String delayUnits,int retryAttempts) throws Exception
    {
        bundle=new Bundle(bundle,UA3ColoHelper);
        displayInputs("testRetryInProcessUpdate", delay, retryType, retryAttempts);
        try {
...
//        List<String> initialData=Util.getHadoopLateData(UA3ColoHelper,Util.getInputFeedFromBundle(bundle));
        ...
        if(retryAttempts<=0 || delay<=0)
        {
            ...
            System.out.println("going to update process at:"+DateTime.now(DateTimeZone.UTC));
            Assert.assertTrue(prismHelper.getProcessHelper().update(Util.readEntityName(bundle.getProcessData()),bundle.getProcessFilePath()).getMessage().contains("updated successfully"),"process was not updated successfully");
            String newBundleId=instanceUtil.getLatestBundleID(UA3ColoHelper,Util.readEntityName(bundle.getProcessData()),"process");
    ...
    @Test(dataProvider="DP",groups={"0.2.2","retry"},enabled=false)
    public void testRetryInProcessHigherDelayUpdate(Bundle bundle,String retryType,int delay,String delayUnits,int retryAttempts) throws Exception
    {
        bundle=new Bundle(bundle, UA3ColoHelper);
        displayInputs("testRetryInProcessUpdate", delay, retryType, retryAttempts);
        try {
...
//        List<String> initialData=Util.getHadoopLateData(UA3ColoHelper,Util.getInputFeedFromBundle(bundle));
        ...
        if(retryAttempts<=0 || delay<=0)
        {
            ...
            System.out.println("going to update process at:"+DateTime.now(DateTimeZone.UTC));
            Assert.assertTrue(prismHelper.getProcessHelper().update(Util.readEntityName(bundle.getProcessData()),bundle.getProcessData()).getMessage().contains("updated successfully"),"process was not updated successfully");
            String newBundleId=instanceUtil.getLatestBundleID(UA3ColoHelper,Util.readEntityName(bundle.getProcessData()),"process");

            Assert.assertEquals(bundleId,newBundleId,"its creating a new bundle!!!");

            //now to validate all failed instances to check if they were retried or not.
            validateRetry(UA3ColoHelper,bundleId,bundle.getProcessObject().getRetry().getAttempts());
            //validateRetry(newBundleId,bundle.getProcessObject().getRetry().getAttempts());
            if(bundle.getProcessObject().getRetry().getAttempts()>0)
            {
                checkIfRetriesWereTriggeredCorrectly(UA3ColoHelper,retryType, delay, bundleId);
    ...
    @Test(dataProvider="DP",groups={"0.2.2","retry"},enabled=false)
    public void testRetryInProcessLowerDelayUpdate(Bundle bundle,String retryType,int delay,String delayUnits,int retryAttempts) throws Exception
    {
        bundle=new Bundle(bundle,UA3ColoHelper);
        displayInputs("testRetryInProcessUpdate", delay, retryType, retryAttempts);
        try {
...
//        List<String> initialData=Util.getHadoopLateData(UA3ColoHelper,Util.getInputFeedFromBundle(bundle));
   ...
   @Test(dataProvider="DP",groups={"0.2.2","retry"},enabled=false)
    public void testRetryInProcessZeroDelayUpdate(Bundle bundle,String retryType,int delay,String delayUnits,int retryAttempts) throws Exception
    {
        bundle=new Bundle(bundle,UA3ColoHelper);
        displayInputs("testRetryInProcessUpdate", delay, retryType, retryAttempts);
        try {
...
//        List<String> initialData=Util.getHadoopLateData(UA3ColoHelper,Util.getInputFeedFromBundle(bundle));
        ...
        if(retryAttempts<=0 || delay<=0)
        {
            ...
            System.out.println("going to update process at:"+DateTime.now(DateTimeZone.UTC));
            Assert.assertFalse(prismHelper.getProcessHelper().update(Util.readEntityName(bundle.getProcessData()),bundle.getProcessData()).getMessage().contains("updated successfully"),"process was updated successfully!!!");
            String newBundleId=instanceUtil.getLatestBundleID(UA3ColoHelper,Util.readEntityName(bundle.getProcessData()),"process");
    ...
    @Test(dataProvider="DP",groups={"0.2.2","retry"},enabled=false)
    public void testRetryInSimpleFailureCase(Bundle bundle,String retryType,int delay,String delayUnits,int retryAttempts) throws Exception
    {
       bundle=new Bundle(bundle,UA3ColoHelper);
...
//        List<String> initialData=Util.getHadoopLateData(UA3ColoHelper,Util.getInputFeedFromBundle(bundle));
    ...
    @Test(dataProvider="DP",groups={"0.2.2","retry"},enabled=false)
    public void testUserRetryWhileAutomaticRetriesHappen(Bundle bundle,String retryType,int delay,String delayUnits,int retryAttempts) throws Exception
    {
        bundle=new Bundle(bundle,UA3ColoHelper);
...
//        List<String> initialData=Util.getHadoopLateData(UA3ColoHelper,Util.getInputFeedFromBundle(bundle));
        ...
        if(retryAttempts<=0 || delay<=0)
        {
            ...
            for(int i=0;i<1;i++)
            {
                prismHelper.getProcessHelper().getProcessInstanceRerun(Util.readEntityName(bundle.getProcessData()),"?start="+formatter.print(startDate).replace("/","T")+"Z" +"&end="+formatter.print(endDate).replace("/","T")+"Z");
                //Thread.sleep(20000);
            }           
            //now to validate all failed instances to check if they were retried or not.
            validateRetry(UA3ColoHelper,bundleId,bundle.getProcessObject().getRetry().getAttempts());
    ...
    @Test(dataProvider="DP",groups={"0.2.2","retry"},enabled=false)
    public void testUserRetryAfterAutomaticRetriesHappen(Bundle bundle,String retryType,int delay,String delayUnits,int retryAttempts) throws Exception
    {
        bundle=new Bundle(bundle,UA3ColoHelper);
...
//        List<String> initialData=Util.getHadoopLateData(UA3ColoHelper,Util.getInputFeedFromBundle(bundle));
        ...
        else
        {
            ...
            ProcessInstancesResult piResult=prismHelper.getProcessHelper().getProcessInstanceRerun(Util.readEntityName(bundle.getProcessData()),"?start="+formatter.print(dateBoundaries[0]).replace("/","T")+"Z" +"&end="+formatter.print(dateBoundaries[dateBoundaries.length-1]).replace("/","T")+"Z");

            Assert.assertEquals(piResult.getStatusCode(),0,"rerun failed miserably! you fool!");

            System.out.println();

            validateRetry(UA3ColoHelper,bundleId,bundle.getProcessObject().getRetry().getAttempts()+1);
    ...
    @Test(dataProvider="DP",groups={"0.2.2","retry"},enabled=false)
    public void testRetryInSuspendedAndResumeCaseWithLateData(Bundle bundle,String retryType,int delay,String delayUnits,int retryAttempts) throws Exception
    {
        bundle=new Bundle(bundle,UA3ColoHelper);
...
//        List<String> initialData=Util.getHadoopLateData(UA3ColoHelper,Util.getInputFeedFromBundle(bundle));
        ...
        else
        {
            ...
            Util.assertSucceeded(UA3ColoHelper.getProcessHelper().suspend(URLS.SUSPEND_URL,bundle.getProcessData()));


            HashMap<String,Integer> initialMap=getFailureRetriesForEachWorkflow(UA3ColoHelper,getDefaultOozieCoord(UA3ColoHelper,bundleId));
            ...
            HashMap<String,Integer> finalMap=getFailureRetriesForEachWorkflow(UA3ColoHelper,getDefaultOozieCoord(UA3ColoHelper,bundleId));
            ...
            Util.assertSucceeded(UA3ColoHelper.getProcessHelper().resume(URLS.RESUME_URL,bundle.getProcessData()));
    ...
    @Test(dataProvider="DP",groups={"0.2.2","retry"},enabled=false)
    public void testRetryInLateDataCase(Bundle bundle,String retryType,int delay,String delayUnits,int retryAttempts) throws Exception
    {
        bundle=new Bundle(bundle, UA3ColoHelper);
...
//        List<String> initialData=Util.getHadoopLateData(UA3ColoHelper,Util.getInputFeedFromBundle(bundle));
        ...
        else
        {
            ...
            while(true)
            {
                ...
                if(!inserted && validateFailureRetries(UA3ColoHelper,getDefaultOozieCoord(UA3ColoHelper,bundleId), bundle.getProcessObject().getRetry().getAttempts()))
                {
                    System.out.println("inserting data in folder "+insertionFolder+" at "+DateTime.now());
                    Util.injectMoreData(UA3ColoHelper,insertionFolder,"src/test/resources/OozieExampleInputData/lateData");
                    inserted=true;
                    break;
                }
    ...
    @Test(dataProvider="DP",groups={"0.2.2","retry"},enabled=false)
    public void testRetryInDeleteAfterPartialRetryCase(Bundle bundle,String retryType,int delay,String delayUnits,int retryAttempts) throws Exception
    {
       bundle=new Bundle(bundle, UA3ColoHelper); 
...
//        List<String> initialData=Util.getHadoopLateData(UA3ColoHelper,Util.getInputFeedFromBundle(bundle));
        ...
        if(retryAttempts<=0 || delay<=0)
        {
            ...
            validateRetry(UA3ColoHelper,bundleId,(bundle.getProcessObject().getRetry().getAttempts())/2);
            
            Util.assertSucceeded(prismHelper.getProcessHelper().delete(URLS.DELETE_URL,(bundle.getProcessData())));
            
            if(retryType.equals("exp-backoff"))
            {
                Thread.sleep(1000*60*delay*((retryAttempts-(bundle.getProcessObject().getRetry().getAttempts())/2)^2));
            }
            else
            {
                Thread.sleep(1000*60*delay*((bundle.getProcessObject().getRetry().getAttempts())-(bundle.getProcessObject().getRetry().getAttempts())/2));
            }
    ...
    private void validateRetry(ColoHelper coloHelper,String bundleId,int maxNumberOfRetries) throws Exception
    {
    ...
    private boolean validateFailureRetries(ColoHelper coloHelper,CoordinatorJob coordinator,int maxNumberOfRetries) throws Exception
    {
        ...
        for(CoordinatorAction action:coordinator.getActions())
        {
          ...
          if(!(actionInfo.getStatus().equals(WorkflowJob.Status.SUCCEEDED) || actionInfo.getStatus().equals(WorkflowJob.Status.RUNNING)))
          {
              ...
              System.out.println("workflow "+actionInfo.getId()+" has action number: "+actionInfo.getRun());
    ...
    public Object[][] getData() throws Exception
    {
        ...
        Integer[] delays=new Integer[]{2,0};//0,-1,3 //removing -1 since this should be checked at validation level while setting
        ...
        Object[][] testData=new Object[retryTypes.length*delays.length*delayUnits.length*bundleData.length*retryAttempts.length][5];
    	...
    	private boolean allRelevantWorkflowsAreOver(ColoHelper coloHelper,String bundleId,String insertionFolder) throws Exception
	{
			...
			if(job.getAppName().contains("DEFAULT"))
			{
				...
				for(CoordinatorAction action:coordJob.getActions())
				{
					...
					if(actionMan.getRunConf().contains(insertionFolder))
					{
                                            ...
                                            {
                                                System.out.println("related workflow "+actionMan.getId()+" is over....");
                                                finished&=true; 
                                            }
                                            else
                                            {
        ...
        private boolean ensureAllFailedInstancesHaveRetried(ColoHelper coloHelper,String bundleId,int maxNumberOfRetries) throws Exception
        {
        ...
        private void waitTillCertainPercentageOfProcessHasStarted(ColoHelper coloHelper,String bundleId,int percentage) throws Exception
        {
        ...
        private void waitTillCertainPercentageOfProcessAreKilled(ColoHelper coloHelper,String bundleId,int percentage) throws Exception
        {
    ...
    private HashMap<String,Integer> getFailureRetriesForEachWorkflow(ColoHelper coloHelper,CoordinatorJob coordinator) throws Exception
    {
    ...
    private void checkIfRetriesWereTriggeredCorrectly(ColoHelper coloHelper,String retryType,int delay,String bundleId) throws Exception
    {
    ...
    private void checkRetryTriggerForCoordinator(ColoHelper coloHelper,String retryType,int delay,CoordinatorJob coordinator) throws Exception
    {
        ...
        { 
            ...
            if(!coordAction.getStatus().equals(CoordinatorAction.Status.SUCCEEDED))
            {
                ...
                System.out.println("checking timelines for retry type "+retryType+" for delay "+expectedDelay+" for workflow id: "+action.getExternalId());
                ...
                if(retryType.equalsIgnoreCase("periodic"))
                {
                    ...
                    for(int i=0;i<instanceFinishTimes.size()-1;i++)
                    {
                      ...
                      Assert.assertEquals(temp.plusMinutes(expectedDelay).getMillis(), formatter.parseDateTime(instanceRetryTimes.get(i)).getMillis(), 5000, "oops! this is out of expected delay range for workflow id  "+action.getExternalId());
