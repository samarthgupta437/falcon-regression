...
public class RetentionTest {
    ...
    public void testRetention(Bundle bundle, String period, String unit, boolean gaps,
                              String dataType)
    throws Exception {
        ...
        try {
            ...
            String feed = setFeedPathValue(Util.getInputFeedFromBundle(ua2Bundle),
            ...
            if (Integer.parseInt(period) > 0) {
                Util.assertSucceeded(prismHelper.getFeedHelper()
                        .submitEntity(URLS.SUBMIT_URL, Util.getInputFeedFromBundle(ua2Bundle)));

                replenishData(UA2ColoHelper, dataType, gaps);

                Util.CommonDataRetentionWorkflow(UA2ColoHelper, ua2Bundle, Integer.parseInt(period),
                        unit);
            } else {
    ...
    public Object[][] getTestData(Method m) throws Exception {
        ...
        String[] periods = new String[]{"0", "10080", "60", "8",
                ...
                // the system does not
        // allow setting it. Should be covered in validation scenarios.
...
*/
        Object[][] testData =
                new Object[bundles.length * units.length * periods.length * gaps.length *
                        dataTypes.length][5];
