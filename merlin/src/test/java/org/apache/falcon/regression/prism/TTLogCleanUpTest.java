...
public class TTLogCleanUpTest {
    ...
    public void feedReplicaltionLogCleanUp() throws Exception {
        ...
        //when test case starts tomcat has been force restarted , so log cleanup service should
        // have just run.
        Thread.sleep(30000);
        ...
        try {
            ...
            //	instanceUtil.putFileInFolders(ua2,instanceUtil.createEmptyDirWithinDatesAndPrefix
            // (ua2,
            // instanceUtil.oozieDateToDate("2012-10-01T12:04Z"), instanceUtil.oozieDateToDate
            // ("2012-10-01T12:10Z"),
            // "/logCleanUpTest/", 1),"thriftRRMar0602.gz");


            String feed = b1.getDataSets().get(0);
            feed = InstanceUtil.setFeedFilePath(feed,
                    "/logCleanUpTest/${YEAR}/${MONTH}/${DAY}/${HOUR}/${MINUTE}/");
            ...
            InstanceUtil.waitTillParticularInstanceReachState(ua1, Util.getFeedName(feed), 0,
                    org.apache.oozie.client.CoordinatorAction.Status.SUCCEEDED, 20,
                    ENTITY_TYPE.FEED);
            InstanceUtil.waitTillParticularInstanceReachState(ua3, Util.getFeedName(feed), 0,
                    org.apache.oozie.client.CoordinatorAction.Status.SUCCEEDED, 20,
                    ENTITY_TYPE.FEED);
	...
	/*		if(m.getMinutes() <5){
            ...
            ua1.getFeedHelper()
                    .getProcessInstanceRerun(Util.getFeedName(feed), "?start=2012-10-01T12:05Z");
            ua3.getFeedHelper()
                    .getProcessInstanceRerun(Util.getFeedName(feed), "?start=2012-10-01T12:10Z");
    ...
    public void processLogCleanUp() throws Exception {
        ...
        try {
            ...
            b.setInputFeedDataPath(
                    "/logRetentionTest/process/${YEAR}/${MONTH}/${DAY}/${HOUR}/${MINUTE}");

            InstanceUtil.putFileInFolders(ua3, InstanceUtil
                    .createEmptyDirWithinDatesAndPrefix(ua3,
                            InstanceUtil.oozieDateToDate("2010-01-02T00:40Z"),
                            InstanceUtil.oozieDateToDate("2010-01-02T01:10Z"),
                            "/logRetentionTest/process/", 1),
                    "log_01.txt");
