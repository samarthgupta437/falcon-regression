...
//	}
	...
	public ProcessInstancesResult getRunningInstance(
		...
		String url = this.hostname+URLS.INSTANCE_RUNNING.getValue() + "/" +"process/" + name+"/";

		return instanceUtil.createAndsendRequestProcessInstance(url,null,allColo);
	}

	@Override
	public ProcessInstancesResult getProcessInstanceStatus(String EntityName, String params) throws Exception {


		String url = this.hostname+URLS.INSTANCE_STATUS.getValue() + "/"+"process/" + EntityName + "/";
	...
	public String getProcessInstanceStatusViaCli(
		...
		String command ="";
		if(end!=null)
		 command = BASE_COMMAND + " instance -status -url " + this.hostname + " -processName "+EntityName + " -type process " +" -start " + start + " -end "+end;
		else
		command = BASE_COMMAND + " instance -status -url " + this.hostname + " -processName "+EntityName +" -type process " +" -start " + start ;
	...
	public ProcessInstancesResult getProcessInstanceSuspend(
		...
		String url = this.hostname+URLS.INSTANCE_SUSPEND.getValue() + "/"+"process/"  + EntityName + "/";
	...
	public ProcessInstancesResult getProcessInstanceResume(String EntityName, String params) throws Exception {
		String url = this.hostname+URLS.INSTANCE_RESUME.getValue() + "/" +"process/" + EntityName + "/";
		return instanceUtil.createAndsendRequestProcessInstance(url,params,allColo);

	}

	public ProcessInstancesResult getProcessInstanceKill(String EntityName, String params) throws Exception {
		String url = this.hostname+URLS.INSTANCE_KILL.getValue() + "/"+"process/"  + EntityName + "/";
		return instanceUtil.createAndsendRequestProcessInstance(url,params,allColo);
	...
	public String updateViaCLI(String processName,String newProcessFilePath) throws Exception {
		...
		return Util.executeCommand(BASE_COMMAND + " entity -update -url " + this.hostname + " -type process -name " + processName+" -file "+newProcessFilePath);
	}

	@Override
	public String validateEntityViaCLI(String entityName) throws Exception {

		return Util.executeCommand(BASE_COMMAND + " entity -validate -url " + this.hostname+ " -type process -name " + entityName);
	}

	@Override
	public String submitAndScheduleViaCLI(String filePath) throws Exception {

		return Util.executeCommand(BASE_COMMAND + " entity -submitAndSchedule -url " + this.hostname + " -type process -file " + filePath);
	}

	@Override
	public String scheduleViaCLI(String entityName) throws Exception {

		return Util.executeCommand(BASE_COMMAND + " entity -schedule -url " + this.hostname + " -type process -name " + entityName);
	}

	@Override
	public String resumeViaCLI(String entityName) throws Exception {

		return Util.executeCommand(BASE_COMMAND + " entity -resume -url " + this.hostname + " -type process -name " + entityName);
	}

	@Override
	public String getStatusViaCLI(String entityName) throws Exception {

		return Util.executeCommand(BASE_COMMAND + " entity -status -url " + this.hostname + " -type process -name " + entityName);
	}

	@Override
	public String getEntityDefinitionViaCLI(String entityName) throws Exception {

		return Util.executeCommand(BASE_COMMAND + " entity -definition -url " + this.hostname + " -type process -name " + entityName);
	}

	@Override
	public String deleteViaCLI(String entityName) throws Exception {

		return Util.executeCommand(BASE_COMMAND + " entity -delete -url " + this.hostname + " -type process -name " + entityName);
	}

	@Override
	public String suspendViaCLI(String entityName) throws Exception {

		return Util.executeCommand(BASE_COMMAND + " entity -suspend -url " + this.hostname + " -type process -name " + entityName);
	}

	public ProcessInstancesResult getProcessInstanceRerun(String EntityName, String params) throws Exception {
		String url = this.hostname+URLS.INSTANCE_RERUN.getValue() + "/"  +"process/"+ EntityName + "/";
		return instanceUtil.createAndsendRequestProcessInstance(url,params,allColo);
	...
	public String submitEntityViaCLI(String filePath) throws Exception {
		...
		return Util.executeCommand(BASE_COMMAND + " entity -submit -url " + this.hostname + " -type process -file " + filePath);
	}
	...
	public String getProcessInstanceStatusCLI(
		...
		String command ="";
		if(end!=null)
			command = BASE_COMMAND + " instance -status -url " + this.hostname + " -processName "+processName +" -start " + start + " -end "+end;
		else
			command = BASE_COMMAND + " instance -status -url " + this.hostname + " -processName "+processName +" -start " + start;
	...
	public String getProcessInstanceRunningCLI(String processName) throws Exception {
		String command = BASE_COMMAND + " instance -running -url " + this.hostname + " -processName "+processName ;
		return Util.executeCommand(command);
	}
	...
	public String getProcessInstanceSuspendCLI(String processName,
		...
		String command ="";
		if(end!=null)
			command = BASE_COMMAND + " instance -suspend -url " + this.hostname + " -processName "+processName +" -start " + start + " -end "+end;
		else
			command = BASE_COMMAND + " instance -suspend -url " + this.hostname + " -processName "+processName +" -start " + start ;
	...
	public String processInstanceRerunCLI(String processName,
		...
		String command ="";
		if(end!=null)
			command = BASE_COMMAND + " instance -rerun -url " + this.hostname + " -processName "+processName +" -start " + start + " -end "+end;
		else
			command = BASE_COMMAND + " instance -rerun -url " + this.hostname + " -processName "+processName +" -start " + start;
	...
	public String processInstanceResumeCLI(String processName,
		...
		String command ="";
		if(end!=null)
			command = BASE_COMMAND + " instance -resume -url " + this.hostname + " -processName "+processName +" -start " + start + " -end "+end;
		else
			command = BASE_COMMAND + " instance -resume -url " + this.hostname + " -processName "+processName +" -start " + start;
	...
	public String processInstanceKillCLI(String processName, String start,
		...
		String command ="";
		if(end!=null)
			command = BASE_COMMAND + " instance -kill -url " + this.hostname + " -processName "+processName +" -start " + start + " -end "+end;
		else
			command = BASE_COMMAND + " instance -kill -url " + this.hostname + " -processName "+processName +" -start " + start ;
	...
	{
		return Util.executeCommand(BASE_COMMAND+ " entity -list -url "+this.hostname+" -type process");
	}

	@Override
	public String getDependencies(String entityName) throws Exception {

		return Util.executeCommand(BASE_COMMAND+ " entity -dependency -url "+this.hostname+" -type process -name "+entityName);
	}

	public String getProcessInstanceStatusCLI(String processName,
			String start, String end, int runid, String type) throws Exception {
		String command ="";
		if(end!=null && runid>=0 && type!=null)
			command = BASE_COMMAND + " instance -status -url " + this.hostname + " -processName "+processName +" -start " + start + " -end "+end+ " -runid "+runid+" -type "+type;
		else if(end!=null && runid>=0 && type==null)
			command = BASE_COMMAND + " instance -status -url " + this.hostname + " -processName "+processName +" -start " + start + " -end "+end+ " -runid "+runid;
		else if(end!=null && runid<0 && type==null)
			command = BASE_COMMAND + " instance -status -url " + this.hostname + " -processName "+processName +" -start " + start + " -end "+end;
		else if(end!=null && runid>0 && type!=null)
			command = BASE_COMMAND + " instance -status -url " + this.hostname + " -processName "+processName +" -start " + start + " -end "+end+ " -type "+type;
		else if(end==null && runid>=0 && type!=null)
			command = BASE_COMMAND + " instance -status -url " + this.hostname + " -processName "+processName +" -start " + start + " -runid "+runid+" -type "+type;
		else if(end==null && runid>=0 && type==null)
			command = BASE_COMMAND + " instance -status -url " + this.hostname + " -processName "+processName +" -start " + start + " -runid "+runid;
		else if(end==null && runid<0 && type==null)
			command = BASE_COMMAND + " instance -status -url " + this.hostname + " -processName "+processName +" -start " + start;
		else if(end==null && runid>0 && type!=null)
			command = BASE_COMMAND + " instance -status -url " + this.hostname + " -processName "+processName +" -start " + start + " -type "+type;
    ...
    public ServiceResponse update(String oldEntity, String newEntity) throws Exception {
        ...
        String url = this.hostname+URLS.PROCESS_UPDATE.getValue() + "/"+ Util.readEntityName(oldEntity) ;
