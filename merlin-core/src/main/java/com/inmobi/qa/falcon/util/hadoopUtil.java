...
public class hadoopUtil {
	...
	public static ArrayList<Path> getAllFilesRecursivelyHDFS(
		...
		for(FileStatus stat : stats)
		{
			...
			if(!stat.isDir())
			{
				...
				returnList.addAll(getAllFilesRecursivelyHDFS(new Configuration(),stat.getPath(),ignoreFolders));
			}
		}
	...
	public static long getFileLength(ColoHelper coloHelper, Path fileHDFSLocaltion) throws Exception{
		setSystemPropertyHDFS();
		Configuration conf =hadoopUtil.getHadoopConfiguration(coloHelper);
	...
	public static void copyDataToFolder(ColoHelper coloHelper, final Path folder, final String fileLocation) throws Exception {
		Configuration conf = new Configuration();
		conf.set("fs.default.name", "hdfs://"
	...
	public static ArrayList<String> getAllRecords(Path path) throws Exception{
		ArrayList<Path> rrPaths = hadoopUtil.getAllFilesRecursivelyHDFS(new Configuration(), path,"shouldNOtMatch");
	...
	public static ArrayList<String> getAllRecords(ColoHelper prismHelper,Path path,String... notToMatch) throws Exception{
		Configuration conf=new Configuration();
		conf.set("fs.default.name","hdfs://"+prismHelper.getProcessHelper().getHadoopURL()+"");
		ArrayList<Path> rrPaths = hadoopUtil.getAllFilesRecursivelyHDFS(prismHelper,path,notToMatch);
